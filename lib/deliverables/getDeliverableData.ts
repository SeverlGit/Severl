...
export async function getMonthlyDeliverables(
  ...
  const { data, error } = await supabase
    .from('deliverables')
    .select(
      'id, org_id, client_id, month, type, title, status, assignee_id, due_date, notes, archived_at, clients!inner(brand_name, platforms, tag, retainer_amount, archived_at), team_members(name)'
    )
    .eq('org_id', orgId)
    .eq('month', monthKey)
    .is('archived_at', null)
